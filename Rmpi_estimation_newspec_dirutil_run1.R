...
theta.start <- as.numeric(colMeans(Theta[,1:36]))
qq <- qq[1:36,1:36]
...
tobs <- data.frame(matrix(NA, nrow = 36, ncol = 14))
#for (ss in 1:16) {
for (ss in 1:14) {
  cat(ss)
...
sigma.proposal <- sigma.proposal*(2.38^2)/36

# prior mean
mean.prior <- rep(0,36)
# prior var-covar
sigma.prior <- diag(rep(10,36))

# data frame of all parameters simulations
Theta <- data.frame(matrix(NA, nrow = number_param_simulations, ncol= 36) )
...
save.image("D:/segnet/results/estimation/Rmpi_estimation_newspec_dirutil_run1.RData")