...
InitErgmTerm.indhomophily<-function (nw, arglist, ...) {
  ...
  if(!is.null(attrname)){
    nodecov <- get.node.attr(nw, attrname, "indhomophily")
    u <- NVL(a$levels, sort(unique(nodecov)))
    if(any(is.na(nodecov))){u<-c(u,NA)}
    nodecov <- match(nodecov,u,nomatch=length(u)+1)
    ui <- seq(along=u)
    if (length(u)==1)
      stop ("Attribute given to indhomophily() has only one value", call.=FALSE)
    if (!diff) {
      coef.names <- paste("indhomophily",attrname,sep=".")
      inputs <- c(nodecov)
    } else {
      #  Number of input parameters before covariates equals number of
      #  unique elements in nodecov, namely length(u)
      coef.names <- paste("indhomophily", attrname, u, sep=".")
      inputs <- c(ui, nodecov)
    ...
    coef.names <- "indhomophily"
    inputs <- NULL
  }
  list(name="indhomophily", 
       coef.names=coef.names, 
       pckgname = "ergm.userterms", 
       inputs=inputs,
       dependence = TRUE,
       minval = 0)
}
